...
public class TagRepository {
    ...
    public void replaceTags(long archiveId, List<String> tags) {
        try (Connection connection = databaseManager.getConnection()) {
            ...
            try (PreparedStatement delete = connection.prepareStatement("DELETE FROM tags WHERE archive_id = ?")) {
                delete.setLong(1, archiveId);
                delete.executeUpdate();
            }
            if (tags != null && !tags.isEmpty()) {
                try (PreparedStatement insert = connection.prepareStatement("INSERT INTO tags(archive_id, tag) VALUES (?, ?)")) {
                    for (String tag : tags) {
    ...
    public List<String> findTagsByArchiveId(long archiveId) {
        String sql = "SELECT tag FROM tags WHERE archive_id = ? ORDER BY tag";
